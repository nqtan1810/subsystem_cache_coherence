...
def assembler (tc_id, core_id):
    # fi= open('assembly_code.txt', 'r')
    # sel = int(input("Select testcase: "))
    path_src = f"D:/University/KLTN/hw_design/testcase/assembly/testcase_{core_id}_{tc_id}.asm"
    path_dst = f"D:/University/KLTN/hw_design/testcase/assembly/testcase_{core_id}_{tc_id}.bin"
    fi= open(path_src, 'r')
    string= fi.read()
    fi.close()
    fo= open(path_dst, 'w')
    ...
    for i in range (pos+1,len(ins)) :
        ...
        if FMT[t[0]] == "UJ":
            string= UJType(ins[i])
        # result += (string + '\n')
        hex_string = hex(int(string, 2))[2:].zfill(8).upper()
...
def merge_files_from_lines(file1_path: str, k: int, file2_path: str, h: int, output_path: str) -> None:
    """
    Merges content from two files into a 65536-line output file, starting at specified line indices.
    Content from file2 will overwrite file1 content if both target the same line number.
    Unfilled lines will be filled with '0x00000000'.

    Args:
        file1_path (str): Path to the first input file.
        k (int): Start line index in output file for file1 content (0-based).
        file2_path (str): Path to the second input file.
        h (int): Start line index in output file for file2 content (0-based).
        output_path (str): Path to write the final merged output.
    """
    total_lines = 65536
    default_line = "0x00000000\n"
    output_lines = [default_line] * total_lines
    ...
    # Insert file1 content at index k
    for i, line in enumerate(file1_lines):
        idx = k + i
        if 0 <= idx < total_lines:
            output_lines[idx] = line if line.endswith('\n') else line + '\n'

    # Insert file2 content at index h (overwriting file1 content if overlapping)
...
tc_id = int(input("Enter testcase to init mem: "))
file1_path = f"D:/University/KLTN/hw_design/testcase/assembly/testcase_A_{tc_id}.bin"
file2_path = f"D:/University/KLTN/hw_design/testcase/assembly/testcase_B_{tc_id}.bin"
output_path = "D:/University/KLTN/hw_design/coherence_cache/coherence_cache.srcs/sources_1/new/mem_init.mem"
merge_files_from_lines(file1_path, 0, file2_path, 256, output_path)
